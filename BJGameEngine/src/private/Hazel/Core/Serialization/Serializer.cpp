...
Serializer::RecordData::RecordData(TypeId type,
                                   void *ptr,
                                   const std::string &property,
                                   int32 index)
    : m_Type(type), m_ValuePointer(ptr), key(property), recordIndex(index){};

Serializer::Serializer(SerializeType type) : m_SerializeType(type)
{
...
{
    m_Record.datas.emplace_back(type,
                                target,
                                m_Record.recentKey,
                                m_Record.datas.size());
    onBeginSaveMap(type);
...
void Serializer::Save(const TypeId type, void *data)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (Reflection::IsDerivedClass(Reflection::GetTypeID<BaseObject>(),
                                           info->m_OriginalType) &&
