...
namespace Hazel
{
	...
	ObjectID ObjectDB::Add(BaseObject* p_object)
	{
		...
		{
			HZ_CORE_ASSERT(m_SlotCnt < (1 << OBJECTDB_SLOT_MAX_COUNT_BITS), "ObjectDB::Add() : m_SlotCnt >= m_SlotMax");
			// CRASH_COND(m_SlotCnt == (1 << OBJECTDB_m_SlotMax_COUNT_BITS));

			uint32_t new_m_SlotMax = m_SlotMax > 0 ? m_SlotMax * 2 : 1;

			m_ObjectSlotArray = (ObjectSlot*)m_Allocator.Reallocate(m_ObjectSlotArray, sizeof(ObjectSlot) * new_m_SlotMax);
	...
	void ObjectDB::Remove(BaseObject* p_object)
	{
		...
		if (m_ObjectSlotArray[slot].object != p_object) {
			...
			HZ_CORE_ASSERT(m_ObjectSlotArray[slot].object == p_object, "ObjectDB::Remove() : m_ObjectSlotArray[slot].object != p_object");
		}
		...
		if (m_ObjectSlotArray[slot].valid_num != validator)
		{
			...
			HZ_CORE_ASSERT(m_ObjectSlotArray[slot].valid_num == validator, "ObjectDB::Remove() : m_ObjectSlotArray[slot].valid_num != validator");
		}
