...
namespace Hazel
{
	...
	namespace Utils
	{
		...
		static void CreateFrameBufferTextures(bool multiSampled, uint32_t* outID, uint32_t count)
		{
		...
		static void AttachDepthFrameBufferTexture(uint32_t id, int samples, GLenum format,
	...
	void OpenGLFrameBuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecs.size() > 0)
		{
			...
			Utils::CreateFrameBufferTextures(multiSample, m_ColorAttachmentIDs.data(), m_ColorAttachmentIDs.size());
		...
		if (m_DepthAttachmentSpec.m_TextureFormat != FrameBufferTextureFormat::None)
		{
			...
			{
				Utils::AttachDepthFrameBufferTexture(m_DepthAttachmentID, m_Specification.Samples, GL_DEPTH24_STENCIL8, GL_DEPTH_STENCIL_ATTACHMENT,
