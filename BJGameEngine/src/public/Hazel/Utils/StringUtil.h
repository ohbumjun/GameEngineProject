...
template<typename U, typename std::enable_if<std::is_same<U, wchar_t>::value>::type* = nullptr>
inline U pr_str_tolower(const U c)
{
    return towlower(c);
}
...
public:
    ...
    static char* ReplaceStr(char const* const pattern, char const* const replace, char const* const src);
    ...
    */
    ...
    static void ReplaceWString(std::wstring& str, char target, char replacement);

    static std::vector<std::string> p_split_string(const std::string& origin, char split);
