...
Engine 혹은 Client 측에서 플랫폼에 관계없이
동일한 키코드를 사용하게 하기 위함이다.

예를 들어 Engine 측에서 if (Input::KeyPressed(GLFW_KEY_0)) 이라는
코드를 사용하게 되는 순간, 해당 엔진은 Input 에 대해
openGL 에만 의존하게 된다는 것이다.

이보다는 if (Input::KeyPressed(HZ_KEY_0)) 라는
고유의 key code 를 사용해서
플랫폼 종류에 관계없이 Input 을 받을 수 있게 처리하고자 하는 것이다.
*/
