...
namespace Project
{
	...
	struct LV_EDITOR_API LvFileManagerEventArgs
	{
		...
		LvFileManagerEventArgs(LvFileManagerChangeType type, const LvString& absoluteSourcePath,
															 const LvString& absoluteTargetPath)
																 : type(type), absoluteSourcePath(absoluteSourcePath.c_str()), absoluteTargetPath(absoluteTargetPath.c_str())
		{}
	};
}
