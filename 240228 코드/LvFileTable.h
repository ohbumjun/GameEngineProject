...
public:
    std::function<bool(const char *path)> isFileExist =
        nullptr; // 파일이 존재 확인.
    std::function<bool(const char *path)> isDirectoryExist =
        nullptr; // 디렉터리 유/무.
    std::function<LvString(const char *path)> getPathParent =
        nullptr; // 부모 디렉토리 패스를 출력합니다.
    std::function<LvList<LvString>(const char *path)> getDirectoryLists =
        nullptr; // 디렉토리의 하위의 모든 파일과 디렉터리들을 리턴합니다.
    std::function<LvString(const char *path, const char *path2)> pathCombine =
        nullptr; // 두개의 경로를 경로 구분자와 함께 합성합니다.
    std::function<LvString(const char *path)> getPathName =
        nullptr; // path 의 파일명을 리턴합니다.
    ...
    const LvList<LvString> &GetChildAtAbsolutePaths(
        const char *absolutePath) const;

    const LvFileInfo &CreateRoot(const char *rootAbsolutePath,
                                 const LvList<LvString> &targetPaths = {});
    bool RemoveRecord(const char *absolutePath, bool checkExistFile);
    bool MoveRecord(const char *oldAbsolutePath,
...
private:
    Info *createInstance(const char *parentAbsolutePath,
                         const char *absolutePath,
                         LvList<LvString> *creates);
