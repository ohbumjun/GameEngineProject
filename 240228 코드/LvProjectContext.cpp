...
LvProjectContext::LvProjectContext(const char* absolutePath)
	: project(nullptr)
{
	builtinDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::builtinresource);
	programDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::program);
	settingDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::settings);
	assetCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::asset });
	assetPreviewCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::assetPreview });
	nodeGraphLayoutCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::nodeGraphLayout });
	reflectCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::reflect });
	shaderCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::shader });
	builtInShaderCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::builtInShader });
	infCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::inf });
	platformAssetCacheDirectoryPath = lv_path_combine(absolutePath, { LvEditorContext::Directories::library, LvEditorContext::Directories::Cache::platformAsset });
	solutionDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::solution);
	resourcesDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::resources);
	bundleDirectoryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::bundle);
}

LvProject* LvProjectContext::Initialize(const char* absolutePath, std::function<void()>&& onDestoryed)
{
	...
	{
		return lv_project_get_context()->project->GetPlayState() == LvProject::PlayState::PLAY;
	};
	...
	s_projectContext->project	= new LvProject(absolutePath, std::move(onDestoryed));
	...
	if (false == LvProjectContext::s_isTest)
	{
		...
		editorContext->settings.autoCreateProject = lv_path_parent(absolutePath);
		editorContext->project = project;

		const int index = editorContext->settings.lastOpenProjects.IndexOf(absolutePath);
		if (-1 != index)
		{
		...
		{
			engineContext->resourcesDirectoryPath = lv_path_combine(absolutePath, LvEngineContext::Directories::resources);
			engineContext->sceneGraphPath = s_projectContext->settings.defaultSceneGraphFileName;
			engineContext->bundleDirectoryPath = lv_path_combine(absolutePath, LvEngineContext::Directories::bundle);
		}

		const LvString libraryPath = lv_path_combine(absolutePath, LvEditorContext::Directories::library);
		...
		if (!lv_directory_exist(s_projectContext->assetCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->assetCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->assetPreviewCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->assetPreviewCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->nodeGraphLayoutCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->nodeGraphLayoutCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->reflectCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->reflectCacheDirectoryPath.c_str());
		}
		...
		if (!lv_directory_exist(s_projectContext->shaderCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->shaderCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->builtInShaderCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->builtInShaderCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->infCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->infCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->platformAssetCacheDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->platformAssetCacheDirectoryPath.c_str());
		}

		if (!lv_directory_exist(s_projectContext->solutionDirectoryPath.c_str()))
		{
			lv_directory_create(s_projectContext->solutionDirectoryPath.c_str());
		}
...
void LvProjectContext::Finalize(LvProject* project)
{
	...
	{
		LvAsset* sceneAsset = LvAssetDatabase::GetAssetByInstanceId(LvEditorSceneManager::GetActive()->GetContainer()->host->GetInstanceId());
	...
	if (LvAssetProcessor::IsInitialized()) LvAssetProcessor::Finalize();
	...
	{
		if (LvAssetDatabase::IsUsable) LvAssetDatabase::IsUsable = nullptr;
