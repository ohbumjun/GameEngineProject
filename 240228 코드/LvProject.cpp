...
public:
    void Prepare()
    {
        ...
        {
            if (LvString::IsNullOrEmpty(each.value.uuid))
                continue;

            Editor::LvAsset *refAsset =
                LvAssetDatabase::GetAssetByUuid(LvUuid::Parse(each.value.uuid));

            if (refAsset == nullptr)
                continue;
            if (refAsset->IsBuiltIn() || !refAsset->IsLoaded())
                continue;
            ...
            {
                refAsset =
                    LvAssetDatabase::GetAssetByGuid(refAsset->uuid.GetGuid());
            }
        ...
        LvEditorScene *scene =
            static_cast<LvEditorScene *>(asset->GetPrototype());
    ...
    void Reload()
    {
        ...
        if (asset != nullptr)
        {
            ...
            LvEditorScene *scene =
                static_cast<LvEditorScene *>(asset->GetData());

            scene->Initialize();

            LvEditorSceneManager::SetActive(
                scene); // 작업할 scene으로 EditorContext에 할당
        }
    }
...
const char *LvProject::_destroyEditorEntryMethodName =
    "lv_destroy_editor_entry";

const char *LvProject::_createApplicationEntryMethodName =
    "lv_create_app_entry";
const char *LvProject::_destroyApplicationEntryMethodName =
    "lv_destroy_app_entry";
...
static const LvTypeId s_versionAttributeType =
    LV_TYPEOF(LvSerializeVersionAttribute);
...
bool directory_recursive_create(const char *path)
{
    ...
    for (size_t i = 0; i < paths.Count(); i++)
    {
        ...
        {
            if (!lv_directory_create(
                    current
                        .c_str())) // 여기서 실패하면 내부적인 문제로, 더 깊숙하게 생성할 수 없을것으로 판단하여 바로 탈출
            {
                return false;
            }
        }
...
uint32 calcCrc32(const char *path)
{
    ...
    LvFileStream stream =
        lv_file_open(path, LvFileAccess::READ_ONLY, LvFileMode::OPEN);
...
static std::function<LvShaderCache *(LvShader *,
                                     uint8 /*subShaderIndex*/,
                                     uint8 /*shaderPassIndex*/,
                                     Engine::Renderer::LvShaderPermutationHash)>
    cbBackup = Engine::LvShader::GetSafeShaderCache;
static std::function<LvShaderCache *(LvShader *,
                                     uint8 /*subShaderIndex*/,
                                     uint8 /*shaderPassIndex*/,
                                     Engine::Renderer::LvShaderPermutationHash)>
    cb = [](LvShader *shader,
            uint8 subShaderIndex,
            uint8 shaderPassIndex,
            Engine::Renderer::LvShaderPermutationHash hash) -> LvShaderCache * {
    if (shader == nullptr)
        return nullptr;

    LvShaderAsset *asset = static_cast<LvShaderAsset *>(
        LvAssetDatabase::GetAssetByInstanceId(shader->GetInstanceId()));
    if (asset == nullptr)
    {
...
LvProject::LvProject(const char *projectPath,
                     std::function<void()> &&_onAfterReleaseDllResource)
    : onPlayModeChangedEvent(&_playModeChangedEvent),
      onBeforeLoadEvent(&_beforeLoadEvent), onAfterLoadEvent(&_afterLoadEvent),
      onBeforeUnloadEvent(&_beforeUnloadEvent),
      onAfterUnloadEvent(&_afterUnloadEvent), _projectAbsolutePath(projectPath),
      _settingsFilePath(
          lv_path_combine(projectPath,
                          lv_path_combine("Settings", "ProjectSettings.json"))),
      _libraryDirectoryPath(lv_path_combine(_projectAbsolutePath, "Solution")),
      _temporaryLibraryFolderPath(
          getTemporaryLibraryDirectoryPath(projectPath)),
      _onAfterReleaseDllResource(std::move(_onAfterReleaseDllResource)),
      _libraryLoaders{&_applicableLoader, &_workspaceLoader},
      _createMethodNames{_createApplicationEntryMethodName,
                         _createEditorEntryMethodName},
      _destroyMethodNames{_destroyApplicationEntryMethodName,
                          _destroyEditorEntryMethodName},
      _loopMutex(lv_mutex_create()), _loopCondition(lv_condition_create())
{
    if (LvProjectContext::s_isTest)
        return;
    ...
    lv_file_find(_libraryDirectoryPath.c_str(),
                 "*.xcodeproj",
                 childFilePaths,
                 0);
#else
    ...
    {
        _solutionFileName =
            lv_path_name_without_extension(childFilePaths[0].c_str());
    }

    //const LvString projectName = lv_path_name_without_extension(absolutePath);

    _libraryNames[static_cast<uint8>(LibraryType::APPLICATION)].FormatSelf(
        "%s%s",
        _solutionFileName.c_str(),
        "-Native");
    _libraryNames[static_cast<uint8>(LibraryType::EDITOR)].FormatSelf(
        "%s%s",
        _solutionFileName.c_str(),
        "-NativeEditor");
    ...
    if (lv_directory_exist(_temporaryLibraryFolderPath.c_str()))
    {
        if (!lv_directory_remove(_temporaryLibraryFolderPath.c_str()))
        {
            ...
            LV_LOG(warning,
                   "Failed remove %s",
                   _temporaryLibraryFolderPath.c_str());
        }
    }

    LvProjectPreferences::onValueWasUpdated.AddListener(
        &LvProject::onValueWasUpdated,
        this);
    LvFileManager::onDetectEvent.AddListener(&LvProject::onDetectFileManager,
                                             this);
        ...
        while (true)
        {
            ...
            auto action = [&]() {
                LvDispatchQueue::Main().Sync([&]() {
                    if (!lv_editor_get_context()->isSleepMode &&
                        PlayState::STOP == GetPlayState())
                    {
                        const bool sameLibrary =
                            _lastChangeLibraryCrc32[static_cast<uint8>(
                                LibraryType::APPLICATION)] ==
                                calcCrc32(_copiedLibraryPaths
                                              [static_cast<uint8>(
                                                   LibraryType::APPLICATION)]
                                                  .c_str()) &&
                            _lastChangeLibraryCrc32[static_cast<uint8>(
                        ...
                        if (LvFlagUtil::Contains(s_reserve,
                                                 ActionType::CLEAN_BUILD))
                        {
                            // @donghun 파일들이 변경된게 감지 되었지만 라이브러리 파일은 수정된게 없다면 빌드를 재시도
                            if (sameLibrary)
                            {
                                LvProjectGenerator::AsyncCleanBuild(
                                    !lv_editor_get_context()
                                         ->settings.backgroundBuild);
                            }
                        }
                        else if (LvFlagUtil::Contains(s_reserve,
                                                      ActionType::REFRESH))
                        {
            ...
            if (ActionType::NONE != s_reserve &&
                !lv_editor_get_context()->isSleepMode)
            {
                action();
            }

            if (0 == lv_atomic_get(&_loopAtomic))
                break;
        }
...
LvProject::~LvProject()
{
    ...
    LvFileManager::onDetectEvent.RemoveListener(&LvProject::onDetectFileManager,
                                                this);
    LvProjectPreferences::onValueWasUpdated.RemoveListener(
        &LvProject::onValueWasUpdated,
        this);
...
void LvProject::InitializeVersionController()
{
    ...
    const uint64 engineVersion[4] = {engine->version.major,
                                     engine->version.minor,
                                     engine->version.patch,
                                     engine->version.build};
    const uint64 projectVersion[4] = {settings.major,
                                      settings.minor,
                                      settings.patch,
                                      settings.build};

    LvVersionController::Initialize(engineVersion,
                                    projectVersion,
                                    engine->system->executableDirectoryPath,
                                    _projectAbsolutePath.c_str());
}
...
{
    if (lv_path_contains(LvFileManager::ToAbsolutePath(LV_PROGRAM_DIR).c_str(),
...
void LvProject::onChangeSourceCode(const Project::LvFileManagerEventArgs &args)
{
    ...
    if (LvProjectGenerator::buildState.IsIdle() &&
        lv_editor_get_context()->settings.autoBuild)
    {
        ...
        _lastChangeLibraryCrc32[static_cast<uint8>(LibraryType::APPLICATION)] =
            calcCrc32(_copiedLibraryPaths[static_cast<uint8>(
                                              LibraryType::APPLICATION)]
                          .c_str());
        _lastChangeLibraryCrc32[static_cast<uint8>(LibraryType::EDITOR)] =
            calcCrc32(
                _copiedLibraryPaths[static_cast<uint8>(LibraryType::EDITOR)]
                    .c_str());
    }
}
...
void LvProject::onValueWasUpdated(const char *key)
{
    ...
    if (lv_editor_get_context()->settings.preferencesValueWasUpdatedMaxCount <=
        _valueWasUpdatedCount)
    {
...
bool LvProject::loadLibraries()
{
    if (lv_current_main_thread())
    {
        ...
        LvString path;
        path.Append(lv_path_parent(
            _copiedLibraryPaths[static_cast<uint8>(LibraryType::APPLICATION)]
                .c_str()));
        path.Append(';');
        path.Append(lv_path_parent(
            _copiedLibraryPaths[static_cast<uint8>(LibraryType::EDITOR)]
        ...
        return _isLoadedLibraries[static_cast<uint8>(
                   LibraryType::APPLICATION)] &&
               _isLoadedLibraries[static_cast<uint8>(LibraryType::EDITOR)];
    }
    else
    {
        bool result = false;
        LvDispatchQueue::Main().Sync([&] {
            ...
            LvString path;
            path.Append(lv_path_parent(
                _copiedLibraryPaths[static_cast<uint8>(
                                        LibraryType::APPLICATION)]
                    .c_str()));
            path.Append(';');
            path.Append(lv_path_parent(
            ...
            result =
                _isLoadedLibraries[static_cast<uint8>(
                    LibraryType::APPLICATION)] &&
                _isLoadedLibraries[static_cast<uint8>(LibraryType::EDITOR)];
        });
        return result;
...
bool LvProject::loadLibrary(LibraryType type)
{
    ...
    if (_copiedLibraryPaths[index].IsEmpty())
    {
        ...
        if (LibraryType::APPLICATION == type)
            s_scriptChangeAction.Reload();
        if (LibraryType::EDITOR == type)
    ...
    else
    {
        ...
        _isLoadedLibraries[index] = _libraryLoaders[index]->LoadHandle(
            _copiedLibraryPaths[index].c_str(),
            create,
            destory);
...
void LvProject::unloadLibrary(LibraryType type)
{
    ...
    LvManagedJobSystem::UnregistAll(
        _libraryNames[static_cast<int>(type)].c_str());
    const uint8 index = static_cast<uint8>(type);
...
bool check_version(const LvAssetType assetType,
                   LvBuiltInResource::Info &info,
                   const LvString &absPath,
                   const bool compile,
                   const bool import)
{
    // for only shader
    ...
    const LvString &configPath =
        lv_path_combine(systemContext->executableDirectoryPath,
                        LvEngineContext::Directories::BuiltIn::resource);

    LvString assetTypeName = LvEnum::GetName<LvAssetType>(assetType);
    assetTypeName.ToLower();

    const LvString &versionName =
        LvBuiltInResource::MakeIdentifyName(info.path, info.version.latest);
    const LvString &targetPath =
        lv_path_combine(configPath,
                        {assetTypeName.c_str(), versionName.c_str()});
    const LvString orgFile =
        lv_path_combine(configPath, {assetTypeName.c_str(), info.path.c_str()});
    ...
    if (import == false)
    {
        ...
        info.recompile =
            !lv_file_exist(targetPath.c_str()) || compile ||
            isChanged; // for LV_DEVELOPE_RENDER, 전체 재컴파일이 필요한 경우가 있다.
    }
    ...
    const LvString lastestPath =
        lv_path_combine(LvEditorContext::Directories::builtinresource,
                        {assetTypeName.c_str(), versionName.c_str()});
    ...
    if (recompile || import)
    {
        ...
        {
            const LvString lowVersionName =
                LvBuiltInResource::MakeIdentifyName(info.path, ii);
            const LvString relativePath = lv_path_combine(
                LvEditorContext::Directories::builtinresource,
                {assetTypeName.c_str(), lowVersionName.c_str()});
            const LvString lowVersionPath =
                lv_path_combine(absPath, relativePath);
            ...
            if (lv_file_exist(lowVersionPath.c_str()))
            {
                ...
                if (import)
                {
                    ...
                    {
                        const LvString entryBundleName =
                            LvBundleManifestManager::GetEntryBundleName();
                        LvBundleManifest *manifest =
                            LvBundleManifestManager::GetManifest(
                                entryBundleName);
                        LvAsset *asset =
                            LvAssetDatabase::GetAssetByPath(relativePath);
                        LvBundleDependencyController::AddAsset(entryBundleName,
                                                               asset);
                    }
                }
            }
...
bool preloadHLSL(const LvString &userProjectPath)
{
    ...
    const LvString &configPath =
        lv_path_combine(systemContext->executableDirectoryPath,
                        lv_path_combine("builtin", "resource"));

    // E:\Work\Project\build\Debug\BuiltIn\Resource\shader
    const LvString &dirPath = lv_path_combine(configPath, "shader");

    const LvString targetBuiltInPath =
        lv_path_combine(userProjectPath.c_str(),
                        LvEditorContext::Directories::builtinresource);
    const LvString targetBuiltInShaderPath =
        lv_path_combine(targetBuiltInPath.c_str(), "shader");

    lv_directory_copy(targetBuiltInShaderPath.c_str(),
                      dirPath.c_str(),
                      L"\\*.hlsl.inf");
    ...
    while (!stack.IsEmpty())
    {
        ...
        for (uint32 i = 0; i < total; ++i)
        {
            ...
            if (lv_directory_exist(currentPath.c_str()))
            {
                ...
                LvString projectFilePath =
                    LvString::Replace(currentPath, configPath.c_str(), "");
                projectFilePath = lv_path_combine(targetBuiltInPath.c_str(),
                                                  projectFilePath.c_str());
                // abs : E:\Projects\Project_37 + builtin/resource

                //info도 모두 복사
                lv_directory_copy(projectFilePath.c_str(),
                                  currentPath.c_str(),
                                  L"\\*.hlsl.inf");
            ...
            {
                const LvString ext =
                    lv_path_extension_to_lower(currentPath.c_str());
                if (ext == ".hlsl")
                {
                    ...
                    LvString relPath =
                        LvString::Replace(currentPath, configPath.c_str(), "");
                    relPath = lv_path_combine(
                        LvEditorContext::Directories::builtinresource,
                        relPath.c_str());

                    if (LvAssetDatabase::ImportAsset(
                            relPath,
                            true)) // 앞에 builtin 부분 대소문자 구분 함
                    {
                        LvAsset *asset =
                            LvAssetDatabase::GetAssetByPath(relPath);
                        LV_ASSERT(asset->type == LvAssetType::HLSL,
                                  "check it!");
                        {
                            LvHLSLAsset::AddChangedHLSL(
                                static_cast<LvHLSLAsset *>(asset));
                        }
                    }
                }
            }
...
void LvProject::LoadBuiltIn(LvString &state) const
{
    ...
    for (auto &assetType : LvBuiltInResource::Configuration)
    {
        ...
        for (size_t j = 0, max = assetType.value.Count(); j < max; ++j)
        {
            ...
            state.FormatSelf("Load Built In %s ... (%.1f%%)",
                             file.name.c_str(),
                             progress * 100.f);

            // 셰이더는 실시간 컴파일 필요
            const LvString &versionName =
                LvBuiltInResource::MakeIdentifyName(file.path,
                                                    file.version.latest);

            const LvString &assetTypePath =
                lv_path_combine(dirPath, assetTypeName);
            ...
            const LvString &dstPath =
                lv_path_combine(GetAbsolutePath(), relativePath);
            ...
            if (!lv_file_exist(
                    targetInfoPath
                        .c_str())) //install 후 build/builtin 에 info가 없는 resource가 있는경우 에러
            {
                LV_LOG(error,
                       "YOU MUST MAKE BUILTIN INFO FILE %s",
    ...
    while (!stack.IsEmpty())
    {
        ...
        for (i = 0, total = list.Count(); i < total; ++i)
        {
            ...
            LvString relPath =
                LvString::Replace(absPath, dirWithSep.c_str(), "");
    ...
    for (i = 0, total = relPaths.Count(); i < total; ++i)
    {
        ...
        while (*lookahead != '\0')
        {
            if (*(lookahead - 1) == '_' && *lookahead == 'v' &&
                LvLexer::IsNumber(*(lookahead + 1)))
            {
        ...
        const LvAssetType type = LvAssetExtension::GetAssetTypeByExtension(
            lv_path_extension(relPath.c_str()));
...
void LvProject::SaveShaderCacheKey()
{
    ...
    const LvString key = LvString::Format("%04d%02d%02d_%02d%02d%02d",
                                          pLocal->tm_year + 1900,
                                          pLocal->tm_mon + 1,
                                          pLocal->tm_mday,
                                          pLocal->tm_hour,
    ...
    LV_LOG(debug,
           "[%s]ProjectSettings is saved. It will remove cache and compile all "
           "shaders",
           key.c_str());
}
...
{
    const LvString dirPath = LvBuiltInUtil::
        GetEngineBuiltInShaderPath(); // executableDirectoryPath + /builtin/resource/shader
    ...
    const uint32 count = static_cast<uint32>(
        LvBuiltInResource::Configuration[LvAssetType::SHADER].Count());
    for (uint32 ii = 0; ii < count; ++ii)
    {
        const float progress = static_cast<float>(ii) / count;

        LvShaderAsset *shaderAsset =
            static_cast<LvShaderAsset *>(LvBuiltInResource::Get(
                LvAssetType::SHADER,
                static_cast<LvBuiltInResource::ShaderPreset>(ii)));
        LV_ASSERT(shaderAsset != nullptr, "shader asset is nullptr");
        state.FormatSelf("Compiling Shader %s ... (%.1f%%)",
        ...
        for (uint32 passIndex = 0;
             passIndex < shader->GetSubShader(0)->GetPassCount();
             ++passIndex)
        {
            Engine::Renderer::LvShaderPermutationHash hash =
                shader->GetPass(passIndex)->GetDefaultHash();
            const LvShaderCache *cache =
                shaderAsset->GetCache(hash, 0, passIndex);
        ...
        // /executableDirectoryPath + builtin/resource/shader 쪽에 최신 빌트인 셰이더 캐시를 tod
        {
            ...
            const LvString userProjectCachePath =
                shaderAsset->GetAbsoluteCachePath();
            const LvString cacheName =
                lv_path_name(userProjectCachePath.c_str());
            const LvString targetPath =
                lv_path_combine(cachePath.c_str(), cacheName.c_str());
...
void LvProject::LoadSettings() const
{
    ...
    if (!settings.shaderCacheKey.IsEmpty())
    {
        ...
        const LvString libraryPath =
            lv_path_combine(GetAbsolutePath(),
        ...
        const LvString shaderCacheKeyName =
            LvString::Format("ShaderCompile_%s",
                             settings.shaderCacheKey.c_str());
        const LvString shaderCacheKeyPath =
            lv_path_combine(libraryPath, shaderCacheKeyName);

        bool found = false;
        // 파일이 컴파일마다 여러 개 생성되지 않도록 나머지는 지워준다.
        LvList<LvString> fileList =
            lv_directory_lists(libraryPath.c_str(),
                               true,
                               false); // 폴더 제외하고 파일만
        for (const auto &filename : fileList)
        {
            ...
            else if (filename.SubString(0,
                                        LvString("ShaderCompile_").Length()) ==
                     "ShaderCompile_")
            {
                const LvString rmShader =
                    lv_path_combine(libraryPath, filename);
        ...
        if (!found)
        {
            ...
            lv_directory_remove(
                lv_path_combine(
                    ...
                    .c_str()); // built-in shader
            lv_directory_remove(
                lv_path_combine(libraryPath,
...
void LvProject::MigrateVersion()
{
    ...
    const LvString projectPath =
        lv_path_parent(project->resourcesDirectoryPath.c_str());
...
{
    if (_migrated)
        LvVersionController::ShowResult();
}
...
void LvProject::CheckAssets()
{
    ...
    for (auto each : info->subTypes)
    {
        ...
        for (const auto &attrInfo : subInfo->attributes)
        {
            ...
            {
                const auto attr =
                    static_cast<LvAssetAttribute *>(attrInfo.attribute);

                const auto protoInfo =
                    LvReflection::GetTypeInfo(attr->prototype);
                ...
                for (const auto &protoAttrInfo : protoInfo->attributes)
                {
                    if (protoAttrInfo.type == s_versionAttributeType)
                    {
                        ...
                        {
                            const LvSerializeVersionAttribute *verAttr =
                                static_cast<LvSerializeVersionAttribute *>(
                                    protoAttrInfo.attribute);
                            Target target{0, 0};
    ...
    for (const auto &pair : LvAssetDatabaseInternal::GetGuidAssetRelation())
    {
        ...
        if (!manageds.ContainsKey(type))
            continue;
        if (asset->IsBuiltIn())
            continue;
        if (asset->GetResourceFormat() == LvAsset::ResourceFormat::BINARY ||
            asset->GetResourceFormat() == LvAsset::ResourceFormat::TEXT)
            continue;
        ...
        if (targetVersion != asset->header.serializeVersion)
        {
            // 스레드 경쟁 상태에 놓일 수 있음
            LV_LOG(warning,
                   "If this happens, the ProjectLoad thread and the importing "
                   "job thread may be in a race condition.");

            LvVersionController::MigrateResources(
                target.prototype,
                asset->path.c_str(),
                asset->header.serializeVersion);

            LvAssetDatabase::ImportAsset(
                lv_path_combine(LvEngineContext::Directories::resources,
                                asset->path)
                    .c_str());
            ...
            {
                LV_THROW("Unmatched asset version, you should make the version "
                         "migrator %s",
                         asset->path.c_str());
            }
        }
        ...
        {
            LV_LOG(debug,
                   "%s asset cache load fails, so re-record the cache",
                   asset->path.c_str());
...
LvString LvProject::getTemporaryLibraryDirectoryPath(
    const char *projectPath) const
{
    return getTemporaryLibraryDirectoryPath_ForBuildType(projectPath,
                                                         s_buildType);
}

LvString LvProject::getLibraryDirectoryPath_ForBuildType(
...
#else
    return lv_path_combine(projectPath,
                           {"Solution", buildType, "lib", buildType});
#endif
}

LvString LvProject::getTemporaryLibraryDirectoryPath_ForBuildType(
    const char *projectPath,
    const char *buildType) const
{
#if defined(__WIN32__)
    return lv_path_combine(projectPath,
                           {"Solution", "Temp", buildType, buildType});
#else
    return lv_path_combine(projectPath,
                           {"Solution", "Temp", buildType, "lib", buildType});
#endif
}
...
LvString LvProject::copyToTemp(LibraryType type) const
{
    ...
    const LvString cmakeListPath =
        lv_path_combine(_projectAbsolutePath, "CMakeLists.txt");
    ...
    case LibraryType::EDITOR:
        tailFileName =
            "-NativeEditor"; // Project-NativeEditor.dll, libProject-NativeEditor.dylib
    ...
    else
    {
        const int lastSeparator =
            lv_str_last_index_of(_projectAbsolutePath.c_str(),
                                 LV_DIRECTORY_SEPARATOR_CHAR);
        if (lastSeparator == -1)
        {
...
#endif
    const LvString dllSourcePath =
        lv_path_combine(_libraryDirectoryPath.c_str(),
                        dllFileName.c_str()); // TODO
    ...
    while (true)
    {
        ...
        if (0 < failed)
            tempDllFileName.AppendFormat("-%zu", failed);
        ...
        dllDestinationFile = lv_path_combine(_temporaryLibraryFolderPath,
                                             tempDllFileName.c_str());
        ...
        if (!lv_file_copy(dllDestinationFile.c_str(), dllSourcePath.c_str()))
        {
            ...
            LV_LOG(warning,
                   "Warning project dll copy failed (%s)",
                   dllDestinationFile.c_str());
            ++failed;
        }
        else
        {
...
void LvProject::Play()
{
    ...
    if (PlayState::STOP == _playState)
    {
        if (_playMode == PlayMode::BUNDLE)
        {
            ...
            LvFixedString<LV_CHAR_INIT_LENGTH> bundlePlayFolderPath(
                tempDllFolderPath.c_str());
            bundlePlayFolderPath.Append(LV_DIRECTORY_SEPARATOR_CHAR);
            bundlePlayFolderPath.Append(bundlePlayTempFolderName);

            LvFixedString<LV_CHAR_INIT_LENGTH> bundlePlayResourceFolderPath(
                bundlePlayFolderPath);
            bundlePlayResourceFolderPath.Append(LV_DIRECTORY_SEPARATOR_CHAR);
            bundlePlayResourceFolderPath.Append(
            ...
            if (false ==
                lv_directory_exist(bundlePlayResourceFolderPath.c_str()))
            {
            ...
            // BundlePlay/libGLESv2.dll or libGLESv2.dylib (win32 or mac)
            // BundlePlay/libEGL.dll or libEGL.dylib (win32 or mac)
            if (engineContext->renderPlatform ==
                Render::LvRenderPlatform::GLES3)
            {
            ...
            {
                LvString bundlePath =
                    LvBundleManifestManager::GetBundleFullPath(
                        each->GetBundleName(),
                        LvPlatformType::STANDALONE);
                if (!lv_file_exist(bundlePath.c_str()))
                {
                    LvMessagePanel *message =
                        lv_editor_get_window()->CreatePanel<LvMessagePanel>();
                    message->Open("Play Fail", "Not all bundles were built.");
                    return;
                }
            }

            LvString platformDir = lv_path_combine(
                engContext->bundleDirectoryPath.c_str(),
                lv_get_platform_type_to_string(LvPlatformType::STANDALONE));

            // lv_directory_lists는 full path가 아닌 상대경로를 준다.
            LvList<LvString> bundleFolderFileLists =
                lv_directory_lists(platformDir.c_str(), true, false);
            for (size_t bundleFileIndex = 0;
                 bundleFileIndex < bundleFolderFileLists.Count();
                 ++bundleFileIndex)
            {
                const LvString &bundleFolderFileName =
                    bundleFolderFileLists[bundleFileIndex];

                if (true ==
                    lv_path_check_extension(bundleFolderFileName.c_str(),
                                            LvAssetBundle::BundleExtension))
                {
                    ...
                    fullBundlePath.Append(lv_get_platform_type_to_string(
                        LvPlatformType::STANDALONE));
                    fullBundlePath.Append(LV_DIRECTORY_SEPARATOR_CHAR);
            ...
            for (size_t sourceFileIndex = 0;
                 sourceFileIndex < sourceFiles.Count();
                 ++sourceFileIndex)
            {
                const LvFixedString<LV_CHAR_INIT_LENGTH> &sourceFilePath =
                    sourceFiles[sourceFileIndex];
                int sourceFileLastSeparator =
                    lv_str_last_index_of(sourceFilePath.c_str(),
                                         LV_DIRECTORY_SEPARATOR_CHAR);
                ...
                {
                    targetFilePath.Append(sourceFilePath.c_str() +
                                          sourceFileLastSeparator + 1);
                }
            ...
            // Build Settings File
            {
                ...
                buildSettings.sceneGraphFileName =
                    projectSetting.defaultSceneGraphFileName;
                buildSettings.GPUskinningSetting = projectSetting.isGPUskinning;

                buildSettings.debugWireFrame = projectSetting.debugWireFrame;
                buildSettings.debugVertexNormal =
                    projectSetting.debugVertexNormal;
                ...
                buildSettings.aspect = projectSetting.fixedAspect
                                           ? projectSetting.aspect
                                           : projectSetting.resolution;
...
#define BUILD_SETTING_JSON 1
                ...
                LvFileStream fs(buildSettingFilePath.c_str(),
                                LvFileMode::CREATE);
            ...
            // Scene Graph 파일을 BundlePlay/Resources 폴더로 옮겨준다.
            {
                ...
                LvString targetDirectory =
                    lv_path_directory(targetSceneGraphPath.c_str());
                if (!lv_directory_exist(targetDirectory.c_str()))
                {
                    lv_directory_create(targetDirectory.c_str());
                }

                lv_file_copy(targetSceneGraphPath.c_str(),
                             sourceSceneGraphPath.c_str());
            }
            ...
            // UserSetting 파일도 BundlePlay/Resources 폴더로 옮겨준다.
            {
                ...
                targetUserSettingPath.Append(
                    LvUserSetting::userSettingFileName);
                lv_file_copy(targetUserSettingPath.c_str(),
            ...
            cmdArgs.Add(_copiedLibraryPaths[static_cast<uint8>(
                                                LibraryType::APPLICATION)]
                            .c_str());
            ...
            // destribution
            {
                const LvString &builtinDstDir = lv_path_combine(
                    bundlePlayFolderPath,
                    LvEngineContext::Directories::BuiltIn::distribute);

                if (!lv_directory_exist(builtinDstDir.c_str()))
                {
                    const LvString &builtinSrcDir = lv_path_combine(
                        systemContext->executableDirectoryPath,
                        LvEngineContext::Directories::BuiltIn::distribute);
                    ...
                    for (auto type : Engine::LvBuiltInDistribute::Configuration)
                    {
                        ...
                        const LvString &typeDir =
                            lv_path_combine(builtinDstDir, typeName.c_str());
                        ...
                        for (auto info :
                             Engine::LvBuiltInDistribute::Configuration[type])
                        {
                            LvString name =
                                Engine::LvBuiltInDistribute::MakeIdentifyPath(
                                    type,
                                    info.name,
                                    info.version.latest);
                            ...
                            LvString srcPath =
                                lv_path_combine(builtinSrcDir, path);
                            LvString dstPath =
                                lv_path_combine(builtinDstDir, path);

                            lv_file_copy(dstPath.c_str(), srcPath.c_str());
                        }
                    }

                    LvString srcPath = lv_path_combine(
                        builtinSrcDir,
                        Engine::LvBuiltInDistribute::ConfigFileName);
                    LvString dstPath = lv_path_combine(
                        builtinDstDir,
                        Engine::LvBuiltInDistribute::ConfigFileName);
            ...
            // resource
            {
                ...
                if (!lv_directory_exist(builtinDstDir.c_str()))
                {
                    ...
                    LvString srcPath =
                        lv_path_combine(builtinSrcDir,
                                        LvBuiltInResource::ConfigFileName);
                    LvString dstPath =
                        lv_path_combine(builtinDstDir,
                                        LvBuiltInResource::ConfigFileName);
            ...
            lv_process_create(playEntryExePath.c_str(),
                              cmdArgs.data(),
                              cmdArgs.Count());
        }
        else
        {
            ...
            if (nullptr == _applicable)
            {
                ...
                if (nullptr == _applicable)
                {
                    if (lv_editor_get_context()->settings.disableLoadLibrary)
                    {
                        LV_LOG(
                            error,
                            "The library loading disable option is set, so the "
                            "library was not loaded. (HOW TO FIX : Edit/Editor "
                            "Settings/Disable Library load option OFF)");
                    }
                    else
                    {
                        LV_LOG(error,
                               "Failed to load application library (HOW TO FIX "
                               ": File/Project Build and File/Project Reload)");
                    }
                    return;
                }
            }
            ...
            LvEditorSceneGraph *graph =
                LvEngineAllocator::New<LvEditorSceneGraph>(
                    "Runtime-SceneGraph");
            ...
            {
                LV_LOG(warning,
                       "Application can't play because the current scene of "
                       "SceneGraph is empty");
                Stop();
            }
...
void LvProject::Stop()
{
    ...
    if (mode == PlayMode::EDITOR)
    {
        ...
        if (nullptr != _applicable)
        {
            ...
            LvSceneGraphInternal *afterGraph =
                LvSceneGraphInternal::GetInstance();
...
{
    return _copiedLibraryPaths[static_cast<uint8>(LibraryType::APPLICATION)]
                   .IsEmpty() == false &&
           _copiedLibraryPaths[static_cast<uint8>(LibraryType::EDITOR)]
                   .IsEmpty() == false;
}

bool LvProject::IsLoaded()
{
    return nullptr != lv_project_get_context();
}

LvManifestManagement &LvProject::GetManifestManagement()
{
    return s_manifestManagement;
}
LV_NS_EDITOR_END
