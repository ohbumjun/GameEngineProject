...
typedef LvIntersection (*DetectingIntersect)(const LvBoxBound &bound,
                                             const LvRay &ray,
                                             uint32 id);
typedef LvIntersection (*RankingIntersect)(const LvIntersection &a,
                                           const LvIntersection &b);
...
struct LV_API LvIntersection
{
    ...
    LV_FORCEINLINE LvVec3f GetHitPos(const LvRay &ray) const noexcept
    {
        return ray.origin + ray.dir * t;
    }
    LV_FORCEINLINE operator bool() const noexcept
    {
        return t != LV_FLT_MAX;
    }
    LV_FORCEINLINE LvIntersection &operator=(const LvIntersection &i) noexcept
    {
        t = i.t;
        bound = i.bound;
        id = i.id;
    ...
    LvIntersection operator()(
        const LvBoxBound &bound,
        const LvRay &ray,
        uint32 id,
        DetectingIntersect func = BasicDetectingIntersect) const noexcept;

    //Basic RankingIntersect
    LV_FORCEINLINE static LvIntersection Closest(
        const LvIntersection &a,
        const LvIntersection &b) noexcept
    {
        return a.t < b.t ? a : b;
    }

    static LvIntersection BasicDetectingIntersect(const LvBoxBound &bound,
                                                  const LvRay &ray,
                                                  uint32 id);
};
...
public:
	...
	*/
    LvIntersection Intersect(const LvRay &ray,
                             RankingIntersect ranker) noexcept;

    LV_FORCEINLINE void Clear() noexcept
    {
        _leafCount = 0;
        _leafSize = 4;
        _tree.Clear();
        _detector = LvIntersection::BasicDetectingIntersect;
    }
    LV_FORCEINLINE void SetIntersector(DetectingIntersect intersect) noexcept
    {
        _detector = intersect;
    }
    LV_FORCEINLINE void ClearIntersector()
    {
        _detector = LvIntersection::BasicDetectingIntersect;
    }
    LV_FORCEINLINE void SetLeafSize(uint32 size) noexcept
    {
        _leafSize = size;
    }
    LV_FORCEINLINE constexpr uint32 Count() const noexcept
    {
        return _leafCount;
    }
    LV_FORCEINLINE LvFastLinearTree<Pair> &GetTree() noexcept
    {
        return _tree;
    }
...
public:
    ...
    bool UpdateProxy(int proxyId,
                     const LvBoxBound &aabb,
    ...
    void Query(T *callback, const LvBoxBound &aabb)
    {
        ...
        while (count)
        {
            LV_CHECK(count < _stackCapacity,
                     "count is more than stack capacity");
            // Pop from the stack
