#pragma once
#ifndef __LV_COLLIDER_COMPONENT_H__
#define __LV_COLLIDER_COMPONENT_H__
...
class LV_API
	LV_REFLECT
	LV_ATTRIBUTE(LvShowOrderAttribute(9))
	LV_ATTRIBUTE(LvInspectorNameAttribute("Collider"))
	LV_ATTRIBUTE(LvSerializeVersionAttribute(0))
	LvColliderComponent : public LvComponent
{
...
public:
	...
	*/
	LV_FORCEINLINE const LvBoxBound& GetLocalBound() const { return _localBound; }

	LV_REFLECT
	LV_FORCEINLINE void SetLocalBound(const LvBoxBound& bound)
	{
		if (_localBound == bound) return;
		_localBound = bound;
	 ...
	 */
	LV_FORCEINLINE const LvBoxBound& GetWorldBound() const { return _worldBound; }
...
protected:
	...
	LvBoxBound calculateWorldBound(const LvVec3f& center, const LvVec3f& extent, const LvMat4f& localToWorldMat);


	LV_REFLECT
	LV_ATTRIBUTE(LvInspectorNameAttribute("Local Bound"))
	LV_ATTRIBUTE(LvSetPropertyAttribute("SetLocalBound"))
