...
LvFileTable::Info* LvFileTable::createInstance(const char* parentAbsolutePath, const char * absolutePath, LvList<LvString>* creates)
{	
	// 현재 대상 경로가 루트거나 추적 대상 경로여야 생성
	if (_rootAbsolutePath != absolutePath && !isTargetPath(absolutePath)) return nullptr;

	// 부모경로가 더 낮은 인덱스에 담김
	if (creates) creates->Add(absolutePath);

	Info* parent = nullptr != parentAbsolutePath ? getFileInfoAtAbsolutePath(parentAbsolutePath) : nullptr;
	Info* info = getFileInfoAtAbsolutePath(absolutePath);
	if (nullptr == info)
	{
		info = new(LV_ENGINE_SAFE_ALLOC(Info)) Info(this, parent, absolutePath, creates);
		_infos.Add(info);
...
const LvFileInfo& LvFileTable::CreateRoot(const char* rootAbsolutePath, const LvList<LvString>& targetPaths)
{
...
{
	if (_targetPaths.Count() == 0) return true;

	for (uint64 i = 0, max = _targetPaths.Count(); i < max; i++)
	{
		if (0 == strncmp(absolutePath, _targetPaths[i].c_str(), _targetPaths[i].Length()))
		{
			return true;
		}
	}
...
void LvFileTable::releaseFileInfo(Info * info)
{
	...
	{
		for (int64 i = static_cast<int64>(info->GetChildCount()) - 1; 0 <= i; --i)
		{
...
{
	LV_CHECK(!_absolutePathToInfo.ContainsKey(info->data.absolutePath), "Already registed (%s)", info->data.path.c_str());
...
const LvFileInfo& LvFileTable::GetFileInfoAtAbsolutePath(const char* absolutePath) const
{
...
const LvList<LvString>& LvFileTable::GetChildAtAbsolutePaths(const char * absolutePath) const
{
	const Info* info = getFileInfoAtAbsolutePath(absolutePath);
	if (nullptr != info)
	{
...
bool LvFileTable::CreateRecord(const char* absolutePath, LvList<LvString>* creates)
{
	const bool existInfo = nullptr != getFileInfoAtAbsolutePath(absolutePath);

	// 이미 있으면 기록하지 않음
	if (existInfo) return false;
	...
	{
		LV_LOG(warning, "LvFileTable::CreateRecord Fail - Wrong path %s (Need root path %s)", absolutePath, rootAbsolutePath);
		return false;
	}
#endif
	...
	const bool existParent = nullptr != getFileInfoAtAbsolutePath(parentAbsolutePath.c_str());
	if (!existParent)
	{
		...
		if (!result)
		{
			LV_LOG(warning, "LvFileTable::CreateRecord Fail - Fail create parent info %s", absolutePath);
		}
		return result;
	}

	// 현재 경로에 대한 정보 생성
	const Info* current = createInstance(parentAbsolutePath.c_str(), absolutePath, creates);
	if (nullptr != current)
	{
		return true;
	}
...
bool LvFileTable::ModifyRecord(const char* absolutePath, LvList<LvString>* creates)
{
	const auto info = getFileInfoAtAbsolutePath(absolutePath);

	if (nullptr == info) return CreateRecord(absolutePath, creates);

	if (!info->data.isDirectory) return true;

	const auto list = getDirectoryLists(absolutePath);

	if (info->GetChildCount() == list.Count()) return true;
...
LvString LvFileTable::ToRelativePath(const char* absolutePath) const
{
	...
	if (0 != strncmp(_rootAbsolutePath.c_str(), absolutePath, _rootAbsolutePath.Length())) LV_LOG(warning, "Converting an absolute path that does not match the root (root : %s target : %s)", _rootAbsolutePath.c_str(), absolutePath);

	result.Remove(0, _rootAbsolutePath.Length());
	return result;
}

LvFileTable::Info* LvFileTable::getFileInfoAtAbsolutePath(const char* absolutePath) const
{
	const LvString key = toNormalize(absolutePath);
	if (!_absolutePathToInfo.ContainsKey(key)) return nullptr;
...
bool LvFileTable::RemoveRecord(const char* absolutePath, const bool checkExistFile)
{
	// 파일이나 디렉토리가 있으면 기록하지 않음
	if (checkExistFile && (isFileExist(absolutePath) || isDirectoryExist(absolutePath)))
	{
...
bool LvFileTable::MoveRecord(const char* oldAbsolutePath, const char* newAbsolutePath, LvList<LvString>* creates)
{
...
LvFileTable::Info::Info(LvFileTable* table, Info* parent, const char* absolutePath, LvList<LvString>* creates)
	: data(LvFileInfo{ absolutePath, table })
{
	...
	if (data.isDirectory)
	{
		Lv::LvList<LvString> childs = table->getDirectoryLists(data.absolutePath.c_str());
		for (uint64 i = 0, max = childs.Count(); i < max; i++)
		{
			LvString childAbsolutePath = table->pathCombine(data.absolutePath.c_str(), childs[i].c_str());

			_table->createInstance(data.absolutePath.c_str(), childAbsolutePath.c_str(), creates);
		}
	}

	if (nullptr != _parent)
	{
		LV_CHECK(!_parent->_childAbsolutePaths.Contains(data.absolutePath), "Already regist file info");
...
{
	return _table->getFileInfoAtAbsolutePath(_childAbsolutePaths[index].c_str());
}

const LvFileTable::Info * LvFileTable::Info::FindChild(const char * childName) const
{
