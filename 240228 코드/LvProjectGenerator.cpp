...
bool LvProjectGenerator::Generate(const char *projectPath)
{
    ...
    if (0 != cmakeGenerate(projectPath))
    {
        LV_LOG(
            error,
            "CMake Configuring incomplete (HOW TO FIX : Remove the "
            "'app/CMakeLists.txt' or 'editor/CMakeLists.txt' and try again)");
    }
...
void LvProjectGenerator::AsyncCleanBuild(const char *projectPath,
...
void LvProjectGenerator::CreateProject(const char *projectPath)
{
    ...
    CoreCLR::ProjectManager::CreateEntryCodes(context->executableDirectoryPath,
                                              projectPath);
...
bool LvProjectGenerator::cmakeListsUpdate(const char *projectPath,
                                          bool removeAndUpdate)
{
    static LvSimpleCMakeAST ast;
    auto update = [](const char *cmakelistPath,
                     const char *absolutePath,
                     const LvList<LvString> &ignores,
                     const char *setName) {
    ...
    const LvString sourcePath =
        lv_path_combine(projectPath, LV_PROGRAM_NATIVE_SRC_DIR);
    const LvString applicationPath =
        lv_path_combine(projectPath, LV_PROGRAM_NATIVE_SRC_APP_DIR);
    const LvString editorPath =
        lv_path_combine(projectPath, LV_PROGRAM_NATIVE_SRC_EDITOR_DIR);

    const LvString projectCmakePath =
        lv_path_combine(projectPath, "CMakeLists.txt");
    const LvString applicatioCmakePath =
        lv_path_combine(applicationPath, "CMakeLists.txt");
    const LvString editorCmakePath =
        lv_path_combine(editorPath, "CMakeLists.txt");
    ...
    if (!lv_file_exist(applicatioCmakePath.c_str()) ||
        !lv_file_exist(editorCmakePath.c_str()))
    {
        CoreCLR::ProjectManager::CreateCMakeLists(projectPath);
    }

    update(applicatioCmakePath.c_str(),
...
void LvProjectGenerator::cmakeClean(const char *projectPath)
{
    ...
    const LvString mainCmakeFilesAbsolutePath =
        lv_path_combine(projectPath, {LV_SOLUTION_DIR, cmakeFiles});
    const LvString cmakeCacheAbsolutePath =
        lv_path_combine(projectPath, {LV_SOLUTION_DIR, cmakeCache});
...
int LvProjectGenerator::cmakeGenerate(const char *projectPath)
{
    ...
    LvString args =
        LvString::Format("-DLV_ENGINE_BIN=\"%s\"", lv_path_current());
#else
    LvString args =
        LvString::Format("-DLV_LEAK_CHECK=ON -DLV_ENGINE_BIN=\"%s\"",
                         lv_path_current());
#endif

    return CoreCLR::ProjectManager::GenerateProjectCmake(projectPath,
                                                         args.c_str());
}
...
bool LvProjectGenerator::cmakeCleanAndUpdateAndGenerate(const char *projectPath)
{
    ...
    const int32 id =
        LvProgress::Start("User project", nullptr, ProgressOptions::INDEFINITE);
...
bool LvProjectGenerator::CleanBuild(const char *projectPath,
                                    const bool showProgress)
{
    if (!buildState.IsIdle())
    {
    ...
    if (showProgress)
    {
        LvDispatchQueue("Refresh title").Async([] {
            lv_editor_get_window()->DisplayProgressBar(0.f,
                                                       "",
                                                       "",
                                                       "CleanBuild");
            while (!buildState.IsIdle())
            {
                ...
                const float second =
                    static_cast<float>(current - s_start) / 1000.f;

                s_title.FormatSelf("Build (%.3f sec)", second);

                if (!buildState.IsIdle())
                {
                    lv_editor_get_window()->DisplayProgressBar(s_progress,
                                                               s_title.c_str(),
                                                               s_message);
                }
            }
    ...
    auto isCancel = [&]() { return onCancel || buildState.IsCancel(); };
...
typedef LvApplicable *(LV_CALL *lv_create_app_entry)(
    Render::LvRenderContext *renderContext);

bool LvProjectGenerator::buildReflection()
{
    LvString args = "--build";
    const auto projectContext = lv_project_get_context();
    args = args.AppendFormat(" \"%s\" --config Debug --target",
                             projectContext->solutionDirectoryPath.c_str());

    const LvString reflectionApplication =
        args.AppendFormat(" ReflectionApplication");
    if (0 !=
        CoreCLR::ProjectManager::ExecuteCmake(reflectionApplication.c_str()))
    {
        LV_LOG(error,
               "Reflection Application build failed (HOW TO FIX : Remove the "
               "'Program/native/src/app/CMakeLists.txt' and try again)");
        return false;
    }
    ...
    {
        LV_LOG(error,
               "Reflection Editor build failed (HOW TO FIX : Remove the "
               "'Program/native/src/editor/CMakeLists.txt' and try again)");
        return false;
    }
    return true;
}
