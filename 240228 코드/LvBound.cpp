...
bool LvBoxBound::Intersect(const LvRay &ray) const
{
    ...
    for (int i = 0; i < 3; ++i)
    {
        if (LvMath::Abs(ray.dir[i]) < rayDirEpsilon)
        {
            ...
            if (ray.origin[i] < minp[i] || ray.origin[i] > maxp[i])
                return false;
        }
        else
        {
...
bool LvBoxBound::Intersect(const LvRay &ray, float &tnear) const
{
    ...
    if (tzminp > tzmaxp)
        LV_SWAP(tzminp, tzmaxp, float);

    if (tzminp > tmaxp || tminp > tzmaxp)
        return false;
    if (isinf(tminp) || isinf(tmaxp) || isinf(tzminp) || isinf(tzmaxp))
        return false;
...
{
    return ((this->minp.x < o.maxp.x ||
             LvMath::EpsilonEqual(this->minp.x, o.maxp.x, 1.732e-3f)) &&
            (this->maxp.x > o.minp.x ||
             LvMath::EpsilonEqual(this->maxp.x, o.minp.x, 1.732e-3f)) &&
            (this->minp.y < o.maxp.y ||
             LvMath::EpsilonEqual(this->minp.y, o.maxp.y, 1.732e-3f)) &&
            (this->maxp.y > o.minp.y ||
             LvMath::EpsilonEqual(this->maxp.y, o.minp.y, 1.732e-3f)) &&
            (this->minp.z < o.maxp.z ||
             LvMath::EpsilonEqual(this->minp.z, o.maxp.z, 1.732e-3f)) &&
            (this->maxp.z > o.minp.z ||
             LvMath::EpsilonEqual(this->maxp.z, o.minp.z, 1.732e-3f)));
}
...
bool LvBoxBound::IntersectRayAsLine(const LvRay &ray, float thresold) const
{
    ...
    if (LvVec3f::Length(minp + p21 * ua - (ray.origin + p43 * ub)) > thresold)
        return false;
...
void LvOrientedBoxBound::ExpandToInclude(const LvBound &b)
{
    ...
    case LvBoundType::BOX:
    {
        ...
        LvVec3f v0 =
            LvVec3f(bb.extent.x, bb.extent.y, bb.extent.z) - bb.GetCenter();
        LvVec3f v1 =
            LvVec3f(bb.extent.x, -bb.extent.y, bb.extent.z) - bb.GetCenter();
        LvVec3f v2 =
            LvVec3f(bb.extent.x, bb.extent.y, -bb.extent.z) - bb.GetCenter();
...
bool TestOBBOBB(const LvOrientedBoxBound &a, const LvOrientedBoxBound &b)
{
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ...
        {
            R[(int)i][(int)j] =
                LvVec3f::Dot(a.transform.cols[i], b.transform.cols[j]);
        }
    }
    ...
    t = LvVec3f(LvVec3f::Dot(t, a.transform.cols[0]),
                LvVec3f::Dot(t, a.transform.cols[2]),
                LvVec3f::Dot(t, a.transform.cols[2]));
    ...
    {
        ra = a.extent[i];
        rb = b.extent[0] * AbsR[(int)i][0] + b.extent[1] * AbsR[(int)i][1] +
             b.extent[2] * AbsR[(int)i][2];
        if (LvMath::Abs(t[i]) > ra + rb)
            return false;
    }
    ...
    for (size_t i = 0; i < 3; i++)
    {
        ra = a.extent[0] * AbsR[0][i] + a.extent[1] * AbsR[1][i] +
             a.extent[2] * AbsR[2][i];
        rb = b.extent[i];
        if (LvMath::Abs(t[0] * R[0][i] + t[1] * R[1][i] + t[2] * R[2][i]) >
            ra + rb)
            return false;
    }
...
bool LvOrientedBoxBound::Intersect(const LvRay &ray,
                                   float &tNear,
                                   float &tFar) const
{
    float tMin = LV_FLT_MIN;
...
{
    archive.WriteStartObject(LvReflection::GetTypeId<LvOrientedBoxBound>(),
                             this);
...
{
    archive.ReadStartObject(LvReflection::GetTypeId<LvOrientedBoxBound>(),
                            this);
...
bool LvMeshBound::Intersect(const LvRay &ray) const
{
    ...
    else
    {
        ...
        case LvMesh::MemoryLayout::AOS:
        {
            ...
            size_t vertexSize =
                vertices.Count() / _mesh->GetPolygonVertexCount();
            for (size_t index = 0; index < indices.Count(); index += 3)
            {
                ...
                LvVec3f t0 = _localToWorld * LvVec4f(vertices[vertexIndex0],
                                                     vertices[vertexIndex0 + 1],
                                                     vertices[vertexIndex0 + 2],
                                                     1.f);
                LvVec3f t1 = _localToWorld * LvVec4f(vertices[vertexIndex1],
                                                     vertices[vertexIndex1 + 1],
                                                     vertices[vertexIndex1 + 2],
                                                     1.f);
                LvVec3f t2 = _localToWorld * LvVec4f(vertices[vertexIndex2],
        ...
        case LvMesh::MemoryLayout::SOA:
        {
            ...
            for (size_t index = 0; index < indices.Count(); index += 3)
            {
                ...
                LvVec3f t0 =
                    _localToWorld * LvVec4f(positions[positionIndex0],
                                            positions[positionIndex0 + 1],
                                            positions[positionIndex0 + 2],
                                            positions[positionIndex0 + 3]);
                LvVec3f t1 =
                    _localToWorld * LvVec4f(positions[positionIndex1],
                                            positions[positionIndex1 + 1],
                                            positions[positionIndex1 + 2],
                                            positions[positionIndex1 + 3]);
                LvVec3f t2 =
                    _localToWorld * LvVec4f(positions[positionIndex2],
                                            positions[positionIndex2 + 1],
                                            positions[positionIndex2 + 2],
                                            positions[positionIndex2 + 3]);
...
void LvMeshBound::buildConvex()
{
    ...
    LvList<LvVec3f> vec3Positions(vertexCount, vertexCount);
    {
        ...
        case LvMesh::MemoryLayout::AOS:
        {
            ...
            LvList<float> extremeValues = {vertices[0],
                                           vertices[0],
                                           vertices[1],
                                           vertices[1],
                                           vertices[2],
                                           vertices[2]};
            for (size_t index = 0; index < vertices.Count();
                 index += vertexSize)
            {
        ...
        {
            const LvList<float> &positions = _mesh->GetPositions();
            LvList<float> extremeValues = {positions[0],
                                           positions[0],
                                           positions[1],
                                           positions[1],
                                           positions[2],
    ...
    //remove points in tetrahedron
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            ...
            {
                float distance =
                    LvVec3f::LengthSquared(vec3Positions[initIndices[i]] -
                                           vec3Positions[initIndices[j]]);
        ...
        LV_CHECK(
            maxDistance != LV_EPS,
            "Error"); //every points are same or too small can't calculate convex

        //find basic plane of tetrahedron
        LvLine3D longestLine(vec3Positions[longestSelected[0]],
                             vec3Positions[longestSelected[1]]);
        maxDistance = LV_EPS;
        ...
        LvVec3f center = (vec3Positions[longestSelected[0]] +
                          vec3Positions[longestSelected[1]] +
                          vec3Positions[farthestSelected]) /
                         3.f;
        maxDistance = LV_EPS;
        ...
        LvVec3f triangleNormal = LvVec3f::Normalize(LvVec3f::Cross(
            vec3Positions[longestSelected[0]] - vec3Positions[farthestSelected],
            vec3Positions[longestSelected[1]] -
                vec3Positions[farthestSelected]));
        if (LvVec3f::Dot(triangleNormal, vec3Positions[pointSelected]) +
                LvVec3f::Dot(-triangleNormal,
                             vec3Positions[longestSelected[0]]) >=
            0)
