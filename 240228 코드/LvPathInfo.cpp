...
LvAsset *LvPathInfo::FindAsset(const LvHostAccess &rootHost)
{
    ...
    if (nullptr != comp)
    {
        ...
        return LvAssetDatabase::GetAssetByInstanceId(
            container->host->GetInstanceId());
    }
...
LvPathInfo::LvPathInfo() : _baseTable(nullptr)
{
}

LvPathInfo::LvPathInfo(const char *relativePath,
                       const uint32 localId,
                       const Project::LvFileTable *baseTable)
    : _relativePath(relativePath), _localId(localId), _baseTable(baseTable)
{
    applyPath(_relativePath.c_str());
}

LvPathInfo::LvPathInfo(const LvFileInfo &fileInfo,
                       const Project::LvFileTable *baseTable)
...
LvPathInfo::LvPathInfo(const LvComponentContainable &containable)
    : LvPathInfo(
          LvAssetDatabase::GetAssetByInstanceId(containable.GetInstanceId()))
{
}
...
LvPathInfo &LvPathInfo::operator=(const LvAsset *asset)
{
    ...
    else
    {
        ...
        _relativePath = LvFileInfo::ToRelativePathAtResourcesRelativePath(
            assetPath.c_str());
        applyPath(_relativePath.c_str());
    }
    return *this;
...
bool LvPathInfo::operator==(const LvPathInfo &o) const
{
    ...
    return 0 == strcmp(o._relativePath.c_str(), _relativePath.c_str()) &&
           o._localId == _localId;
}
...
bool LvPathInfo::operator==(const LvAsset *o) const
{
    if (nullptr != o)
    {
        if (0 == strcmp(LvFileInfo::ToRelativePathAtResourcesRelativePath(
                            o->path.c_str())
                            .c_str(),
                        _relativePath.c_str()))
        {
...
void LvPathInfo::Serialize(LvArchive &archive)
{
    LV_CHECK(
        nullptr == _baseTable,
        "Not support base table"); // @donghun 현재 다른 베이스 테이블을 Serialize를 지원하지 않음, 필요 시 구현 필요
...
{
    const LvString relativePathWithoutResources =
        LvFileInfo::ToResourcesRelativePathAtRelativePath(
            _relativePath.c_str());
    LvAsset *asset = LvAssetDatabase::GetAssetByPath(
        LvFileInfo::ToRelativePathAtResourcesRelativePath(
            relativePathWithoutResources));
    if (0 != _localId && asset != nullptr)
        asset = asset->GetSubAssetByLocalId(_localId);
    return asset;
}
...
LvAssetType LvPathInfo::GetAssetType() const
{
    ...
    return LvAssetExtension::GetAssetTypeByExtension(
        lv_path_extension(_relativePath.c_str()));
}
...
void LvPathInfo::applyPath(const char *relativePath)
{
    ...
    if (_relativePath.Length() &&
        _relativePath[0] == LV_DIRECTORY_SEPARATOR_CHAR)
    {
