...
LvCameraComponent::LvCameraComponent()
    : _projectionType(ProjectionType::PERSPECTIVE),
      _renderType(RenderType::BASE), _aspect(1.f),
      _aaMode(AntiAliasingMode::NONE), _nearP(0.3f), _farP(1000.f),
      _fovRadian(LvMath::Radians(45.f)), _orthoSize(5.f), _depth(0.f),
...
{
    LV_PROFILE_ENGINE();
    if (dirtyFlag.ContainsAny(DirtyFlag::FRUSTUM,
                              DirtyFlag::NEAR_FAR_PLANE,
                              DirtyFlag::PROJECTION_TYPE))
    {
        if (_projectionType == LvCameraComponent::ProjectionType::PERSPECTIVE)
        {
            _projectionMatrix =
                LvMat4f::PerspectiveLH(_fovRadian, _aspect, _nearP, _farP);
        }
        else
        {
            ...
            _projectionMatrix =
                LvMat4f::OrthoLH(-width, width, -height, height, _nearP, _farP);
        }
...
void LvCameraComponent::Transform(const LvTransformComponent &comp)
{
    if (comp.dirtyFlag.IsChanged())
    {
        ...
        _viewMatrix[3][0] = -(_viewMatrix[0][0] * camWorldPos.x +
                              _viewMatrix[1][0] * camWorldPos.y +
                              _viewMatrix[2][0] * camWorldPos.z);
        _viewMatrix[3][1] = -(_viewMatrix[0][1] * camWorldPos.x +
                              _viewMatrix[1][1] * camWorldPos.y +
                              _viewMatrix[2][1] * camWorldPos.z);
        _viewMatrix[3][2] = -(_viewMatrix[0][2] * camWorldPos.x +
                              _viewMatrix[1][2] * camWorldPos.y +
                              _viewMatrix[2][2] * camWorldPos.z);
...
LvVec2f LvCameraComponent::WorldToScreenPoint(const LvVec3f &worldPos) const
{
    ...
    viewportPoint = LvVec3f(viewportPoint.x + 1.f,
                            viewportPoint.y + 1.f,
                            viewportPoint.z + 1.f) /
                    2.f;

    LvVec2f v = LvVec2f(viewportPoint.x * _renderTargetResolution.width,
                        viewportPoint.y * _renderTargetResolution.height);
    return v;
...
{
    return LvVec2f(pos.x * _renderTargetResolution.width,
                   pos.y * _renderTargetResolution.height);
}

LvVec3f LvCameraComponent::ScreenToWorldPoint(const LvVec2f &position) const
{
    const LvVec4f viewSpacePickPosition(
        (((2.0f * position.x) / _renderTargetResolution.width) - 1) /
            _projectionMatrix[0][0],
        (((2.0f * position.y) / _renderTargetResolution.height) - 1) /
            _projectionMatrix[1][1],
        1.0f,
        1.0f);
...
LvVec2f LvCameraComponent::ViewportPointToScreen(const LvVec2f &pos) const
{
    ...
    return LvVec2f(
        pos.x * (_renderTargetResolution.width * _viewportRect.width),
...
LvRay LvCameraComponent::ViewportPointToRay(const LvVec2f &pos) const
{
    ...
    LvVec4f direction =
        _projectInverseMatrix * LvVec4f(pos.x, pos.y, 0.f, 1.0f); // to world
...
void LvCameraComponent::SetCameraStack(LvCameraComponent *component,
                                       size_t index)
{
    ...
    {
        _cameraStack[index]->SetBaseCamera(
            nullptr); // @yongsik-im. 정상적으로 동작하는지(Stack에 존재하던 Overlay Camera를 제거했을 때 제거한 카메라의 Base가 null이 되는지) 확인이 필요합니다.
    }

    // component의 RenderType이 Overlay가 아니면 카메라 스택에 추가하지 않는다.
    if (nullptr != component &&
        component->GetRenderType() != LvCameraComponent::RenderType::OVERLAY)
    {
        component = nullptr;
    }
    ...
    if (nullptr !=
        component) //@yongsik-im. component가 null이 아니라면 해당 컴포넌트의 base camera를 현재 카메라로 설정합니다.
    {
