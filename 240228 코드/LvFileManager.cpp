...
{
public:
    LV_FORCEINLINE LvTypeId GetType() const override
    {
        return LvReflection::GetTypeId<LvFileManagerDebugPanel>();
    }
...
{
    LvFileManagerDebugPanel *panel =
        lv_editor_get_window()->CreatePanel<LvFileManagerDebugPanel>();
    if (nullptr != panel)
    {
        panel->Open();
    }
...
LV_REFLECTION_REGIST
{
    ...
    LvReflection::RegistAttribute<LvMenuItemAttribute>(
        LvAttributeTarget::METHOD,
...
LvFileManager::Collection<LvFileManager::WatchData *>
    LvFileManager::_watchDatas;
LvAtomic LvFileManager::_pooling;

LvRawEvent<const LvFileManagerEventArgs &>::Dispatcher
    *LvFileManager::_detectEvent = nullptr;
LvRawEvent<const LvFileManagerEventArgs &> LvFileManager::onDetectEvent(
    &_detectEvent);

LvRawEvent<const LvList<LvFileManagerEventArgs> &>::Dispatcher
    *LvFileManager::_detectFilesEvent = nullptr;
LvRawEvent<const LvList<LvFileManagerEventArgs> &> LvFileManager::
    onDetectFilesEvent(&_detectFilesEvent);
...
void LvFileManager::Initialize(const char *projectPath)
{
    ...
    _table->getDirectoryLists =
        static_cast<LvList<LvString> (*)(const char *)>(lv_directory_lists);
    _table->pathCombine =
        static_cast<LvString (*)(const char *, const char *)>(lv_path_combine);
    ...
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::bundle)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::library)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::project)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::resources)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::engine)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::settings)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::solution)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::program)
            .c_str()));
    _watchingPaths.Add(lv_path_normalize(
        lv_path_combine(_projectPath, LvEditorContext::Directories::builtin)
            .c_str()));
...
void LvFileManager::StartFileWatcher()
{
    ...
    for (size_t i = 0, max = _handler.Count(); i < max; ++i)
    {
        ...
        _watcher[i] = lv_file_watcher_start(flag,
                                            event,
                                            _watchingRootPath[i].c_str(),
                                            _handler[i]);
    }
}
...
bool LvFileManager::DeleteFromAbsolute(const char *target)
{
    ...
    else if (lv_file_exist(target))
    {
        ...
        LV_LOG(warning,
               "Failed file delete [%d] %s : %s",
               returnCode,
               target,
               lv_print_errno(errorCode));
    }
    return false;
}
        ...
        if (lv_directory_exist(source))
        {
            if (samePath || !lv_directory_exist(target))
            {
                ...
                if (length <= strlen(target) &&
                    LV_DIRECTORY_SEPARATOR_CHAR == target[length])
                {
                    ...
                    if (0 ==
                        strcmp(
                            sub,
                            source)) // 여기서 같다면, 상위 폴더가 내부 폴더로 순환이동을 시도하는 것
                    {
                        recursive = true;
                    }
                    delete[] sub;
                ...
                if (!recursive)
                {
                    ...
                    {
                        onWatch(LvFileManagerChangeType::MOVE_RESOURCE,
                                source,
                                target);
                        return true;
                    }
                    LV_LOG(warning,
                           "Failed directory rename %s -> %s",
                           source,
                           target);
                }
                else
                {
                    LV_LOG(warning,
                           "Failed cannot move to child path %s",
                           target);
                }
            }
            else
...
bool LvFileManager::CopyFromAbsolute(const char *source,
                                     const char *target,
                                     LvString *outCopiedPath)
{
        ...
        if (!lv_directory_exist(target))
        {
            if (0 != strcmp(source, target))
            {
                if (lv_directory_copy(target, source))
                {
                    ...
                    if (import)
                    {
                        onWatch(LvFileManagerChangeType::CREATE_RESOURCE,
                                target,
                                "");
                    }
                    else
                    {
                        onWatch(LvFileManagerChangeType::COPY_RESOURCE,
                                source,
                                target);
                    }
    ...
    else if (lv_file_exist(source))
    {
        ...
        {
            targetStrName = LvString::Format("%s (%d)%s",
                                             targetPathWithoutExtension.c_str(),
                                             index++,
                                             targetExtension.c_str());
        }
...
LvFileStream LvFileManager::CreateFileStream(const char *target,
                                             const LvFileMode fileMode)
{
    const LvString absoluteTarget = lv_path_combine(_projectPath, target);
...
LvFileStream LvFileManager::CreateFileStreamFromAbsolute(const char *target,
                                                         LvFileMode fileMode)
{
...
bool LvFileManager::Copy(const char *source,
                         const char *target,
                         LvString *outCopiedPath)
{
    if (nullptr == source)
    ...
    return CopyFromAbsolute(absoluteSource.c_str(),
                            absoluteTarget.c_str(),
                            outCopiedPath);
}
...
bool LvFileManager::Rename(const char *source, const char *target)
{
    ...
    const LvAssetType assetType =
        LvAssetExtension::GetAssetTypeByExtension(lv_path_extension(target));
...
const LvFileInfo &LvFileManager::GetFileInfoAtAbsolutePath(
...
const LvList<LvString> &LvFileManager::GetChildrenAtAbsolutePath(
    const char *absolutePath)
{
    LV_CHECK_MAIN_THREAD();
...
LvString LvFileManager::GetUniqueName(const LvString &name,
                                      const LvString &parentPath,
                                      const LvString &extensionWithDot)
{
    LvString result;

    const LvString parent =
        LvString::IsNullOrEmpty(parentPath)
    ...
    const LvString &nameWithoutEx =
        lv_path_name_without_extension(name.c_str());

    int index = 0;
    do
    {
        result = index == 0 ? LvString::Format("%s%s",
                                               nameWithoutEx.c_str(),
                                               extensionWithDot.c_str())
                            : LvString::Format("%s (%d)%s",
                                               nameWithoutEx.c_str(),
                                               index,
                                               extensionWithDot.c_str());
        result.Trim();
...
LvString LvFileManager::GetClonePath(const LvAsset *asset,
                                     const LvString &name,
                                     const LvString &extensionWithDot)
{
    const LvAsset *mainAsset =
        LvAssetDatabase::GetAssetByGuid(asset->uuid.GetGuid());
    const LvString parentPath = lv_path_parent(mainAsset->path.c_str());
    const LvString uniquePath =
        GetUniqueName(name, parentPath, extensionWithDot);

    return LvString::IsNullOrEmpty(parentPath)
               ? uniquePath
               : lv_path_combine(parentPath, uniquePath);
}
...
bool LvFileManager::isWatchingTargetPath(const char *absolutePath)
{
    for (uint64 i = 0, max = _watchingPaths.Count(); i < max; i++)
    {
        if (0 == strncmp(absolutePath,
                         _watchingPaths[i].c_str(),
                         _watchingPaths[i].Length()))
        {
            return true;
        }
    }
...
{
    LvFileWatcherEventHandler *handler =
        LV_ENGINE_NEW(LvFileWatcherEventHandler);
...
bool LvFileManager::onWatch(LvFileWatcherEventType eventType,
                            const char *src,
                            const char *dst)
{
    LV_CHECK(nullptr != src, "wrong source path");
    if (!isWatchingTargetPath(src))
        return false;
...
bool LvFileManager::onWatch(LvFileManagerChangeType type,
                            const char *src,
                            const char *dst)
{
    switch (type)
    ...
    _watchDatas.Enqueue(new (lv_malloc(sizeof(WatchData)))
                            WatchData(type, src, dst));
    ...
    if (0 == lv_atomic_get(&_pooling))
    {
        LvDispatchQueue::Main().Async([&] {
            ...
            while (_watchDatas.Count())
            {
                ...
                record(watchData->type,
                       watchData->source.c_str(),
                       watchData->destination.c_str());
            }
            dispatch();
            lv_atomic_set(&_pooling, 0);
        });
...
void LvFileManager::record(const LvFileManagerChangeType type,
                           const char *source,
                           const char *destination)
{
    ...
    if (_table != nullptr)
    {
        ...
        case LvFileManagerChangeType::RENAME_RESOURCE:
        {
            if (0 != strcmp(source, destination) &&
                lv_path_compare(source, destination))
            {
                // 대소문자만 변경하는 경우
    ...
    if (result)
    {
        const bool hasDestination =
            LvFileManagerChangeType::MOVE_RESOURCE == type ||
            LvFileManagerChangeType::RENAME_RESOURCE == type ||
            LvFileManagerChangeType::COPY_RESOURCE == type;

        if (0 < creates.Count())
        {
            const bool isDirectroy =
                hasDestination && lv_directory_exist(destination);
            for (size_t i = 0, max = creates.Count(); i < max; ++i)
            {
                if (hasDestination)
                {
                    LvString sourceParent =
                        isDirectroy ? source : lv_path_parent(source);
                    LvString destParent =
                        isDirectroy ? destination : lv_path_parent(destination);
                    LvString original = creates[i];
                    original.Replace(destParent.c_str(), sourceParent.c_str());
                    _dispatchDatas.Add(new (lv_malloc(sizeof(WatchData)))
                                           WatchData(type,
                                                     original.c_str(),
                                                     creates[i].c_str()));
                }
                ...
                {
                    _dispatchDatas.Add(
                        new (lv_malloc(sizeof(WatchData)))
                            WatchData(type, creates[i].c_str(), nullptr));
                }
            }
        }
        else
        {
            _dispatchDatas.Add(new (lv_malloc(sizeof(WatchData)))
                                   WatchData(type, source, destination));
        }
    }
}
...
void LvFileManager::dispatch()
{
    ...
    {
        LvFileManagerEventArgs args(watchData->type,
                                    watchData->source,
                                    watchData->destination);
        ...
        {
            if (nullptr ==
                LvAssetDatabase::GetAssetByUuid(LvUuid::Parse(uuids[j])))
            {
                uuids.RemoveAt(j);
                changed = true;
            }
