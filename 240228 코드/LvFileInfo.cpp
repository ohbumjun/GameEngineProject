...
const LvFileInfo& LvFileInfo::FindChild(const char * childName) const
{
	...
	for (uint64 i = 0, max = childAbsolutePaths.Count(); i < max; i++)
	{
		...
		if (name == childName)
		{			
			return LvFileManager::GetFileInfoAtAbsolutePath(childAbsolutePaths[i].c_str());
		}
	}
...
LvString LvFileInfo::ToResourcesRelativePathAtAbsolutePath(const char * absolutePath)
{
	return lv_path_without_directory(LvEngineContext::Directories::resources, absolutePath);
}

LvString LvFileInfo::ToResourcesRelativePathAtRelativePath(const char * relativePath)
{
	return ToResourcesRelativePathAtAbsolutePath(LvFileManager::ToAbsolutePath(relativePath).c_str());
}

LvString LvFileInfo::ToAbsolutePathAtResourcesRelativePath(const char * relativePath)
{
	return LvFileManager::ToAbsolutePath(lv_path_combine(LvEngineContext::Directories::resources, relativePath).c_str());
}

LvString LvFileInfo::ToRelativePathAtResourcesRelativePath(const char * relativePath)
{
	return lv_path_combine(LvEngineContext::Directories::resources, relativePath);
}

static LvString createUniquePath(const char* targetPath, std::function<bool(const char*)> isDirectory, std::function<bool(const char*)> isExist)
{
	...
	while (number < max)
	{
		if (isFile)
		{
			path = lv_path_combine(parent.c_str(), LvString::Format("%s_%u%s", name.c_str(), number, extension.c_str()).c_str());
		}
		else
		{
			path = lv_path_combine(parent.c_str(), LvString::Format("%s_%u", name.c_str(), number).c_str());
		}
...
{
	return createUniquePath(relativePath,
		[](const char* path)
		{
			return LvFileManager::IsDirectory(path);
		},
		[](const char* path) {
			return LvFileManager::IsExist(path);
		});
}

LvString LvFileInfo::CreateUniqueAbsolutePath(const char* absolutePath)
{
	return createUniquePath(absolutePath,
