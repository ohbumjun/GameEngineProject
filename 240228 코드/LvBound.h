...
/* #include "engine/LvMesh.h" */ namespace Lv { namespace Engine { class LvMesh; } }
...
public:
	...
	virtual bool Intersect(const LvRay& ray, float& tNear, float& tFar) const = 0;
...
public:
	...
	LvBoxBound() noexcept 
		: LvBound(LvBoundType::BOX) {}
	LvBoxBound(const LvVec3f& min, const LvVec3f& max) noexcept
		: LvBound(LvBoundType::BOX), minp(min), maxp(max), extent((max - min) * 0.5f), radius(0.f) {}
	LvBoxBound(const LvVec3f& p) noexcept
		: LvBoxBound(p, p) {}
	LvBoxBound(const LvVec3f& center, const float _radius) noexcept
		: LvBound(LvBoundType::BOX)
		,minp(center - LvVec3f(_radius))
		,maxp(center + LvVec3f(_radius))
		,extent(LvVec3f(_radius))
		,radius(_radius) {}
	LvBoxBound(const LvBoxBound& b) noexcept 
		: LvBound(LvBoundType::BOX), minp(b.minp), maxp(b.maxp), extent(b.extent), radius(b.radius) { }

	LV_FORCEINLINE bool IsValid() noexcept override
	{
		return minp.x != LV_FLT_MAX && minp.y != LV_FLT_MAX && minp.z != LV_FLT_MAX &&
			maxp.x != LV_FLT_MIN && maxp.y != LV_FLT_MIN && maxp.z != LV_FLT_MIN;
	}
	...
	LV_FORCEINLINE LvVec3f GetCenter() const noexcept { return (minp + maxp) * 0.5; }
	LV_FORCEINLINE uint32 MaxDimension() const noexcept
	{
		return extent[0] > extent[1] ?
			(extent[0] > extent[2] ? 0 : 2) : (extent[1] > extent[2] ? 1 : 2);
	}
	...
	{
		minp = b.minp; maxp = b.maxp; extent = b.extent; radius = b.radius; return *this;
	}
...
public:
	...
	LvOrientedBoxBound() noexcept 
		: LvBound(LvBoundType::ORIENTEDBOX) {}
	...
	LvOrientedBoxBound(const LvOrientedBoxBound& b) noexcept
		: LvBound(LvBoundType::ORIENTEDBOX), transform(b.transform), extent(b.extent) {}
	...
	{
		transform = b.transform; extent = b.extent; return *this;
	}
...
public:
	...
	LvMeshBound(LvMesh* mesh, bool isConvex = false)
		: LvBound(LvBoundType::MESH), _mesh(mesh), _isConvex(isConvex) { if(_isConvex) buildConvex(); }

	LvMeshBound(const LvMeshBound& b) noexcept
		: LvBound(LvBoundType::MESH)
		,_mesh(b._mesh)
		,_isConvex(b._isConvex)
		,_convexVertices(b._convexVertices)
		,_convexIndices(b._convexIndices)
	{ }
	...
	void SetConvex(bool isConvex) noexcept { _isConvex = isConvex; }
	void SetWorldMatrix(const LvMat4f& worldMtx) { _localToWorld = worldMtx; }
	bool IsConvex() const noexcept { return _isConvex; }
...
LV_FORCEINLINE static bool Intersect(const LvBoxBound& box1, const LvBoxBound& box2)
{
...
LV_FORCEINLINE static bool IntersectSphere(const LvFrustum& frustum, const LvBoxBound& sphere)
{
	bool result = true;
...
LV_FORCEINLINE static bool IntersectBox(const LvFrustum& frustum, const LvBoxBound& bound)
{
	bool result = true;
...
LV_FORCEINLINE static bool IntersectSIMDAABB(const LvFrustum& frustum, const LvBoxBound& bound)
{
	...
	LvSimd128f MaxRadiusY_0 = lv_simd_mul_add(extentY, lv_simd_abs(planeY0), MaxRadiusX_0);
	LvSimd128f MaxRadius_0 = lv_simd_mul_add(extentZ, lv_simd_abs(planeZ0), MaxRadiusY_0);
	...
	LvSimd128f MaxRadiusY_1 = lv_simd_mul_add(extentY, lv_simd_abs(planeY1), MaxRadiusX_1);
	LvSimd128f MaxRadius_1 = lv_simd_mul_add(extentZ, lv_simd_abs(planeZ1), MaxRadiusY_1);
