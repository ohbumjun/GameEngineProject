...
static char *lv_build_full_path(LvFileWatcher *watcher,
                                FILE_NOTIFY_EXTENDED_INFORMATION *ev)
{
    LvWString fileName(ev->FileName, ev->FileNameLength / 2);

    char ns[LV_CHAR_INIT_LENGTH] = {
        0,
    };
    ...
    size_t totalLen =
        watcher->watchDir.Length() + 1 + normalizedFileName.Length() + 1;
    char *ret =
        (char *)allocator.Alloc(totalLen * sizeof(char), __FILE__, __LINE__);
    memcpy(ret, watcher->watchDir.c_str(), watcher->watchDir.Length());
    ret[watcher->watchDir.Length()] = LV_DIRECTORY_SEPARATOR_CHAR;
    memcpy(ret + watcher->watchDir.Length() + 1,
...
LvFileWatcher *lv_file_watcher_create(LvFileWatcherCreateFlags flags,
                                      LvFileWatcherEventType types,
                                      const char *path)
{
    LvFileWatcher *w = (LvFileWatcher *)allocator.Alloc(sizeof(LvFileWatcher),
                                                        __FILE__,
                                                        __LINE__);
    ...
    w->directory = ::CreateFile(
        ...
        FILE_FLAG_BACKUP_SEMANTICS | FILE_FLAG_OVERLAPPED |
            FILE_FLAG_SEQUENTIAL_SCAN, // file attributes details in https://docs.microsoft.com/en-us/windows/win32/api/fileapi/nf-fileapi-createfilea
...
void lv_file_watcher_get_event(LvFileWatcher *watcher,
                               LvList<LvFileWatcherEventSet> *watchDatas)
{
    DWORD bytes;
    ...
    FILE_NOTIFY_EXTENDED_INFORMATION *notifyInfo =
        reinterpret_cast<FILE_NOTIFY_EXTENDED_INFORMATION *>(
            watcher->readBuffer);
    bool isLoop = false;
    ...
    do
    {
        ...
        notifyInfo = reinterpret_cast<FILE_NOTIFY_EXTENDED_INFORMATION *>(
            (reinterpret_cast<char *>(notifyInfo) +
        ...
        if (!lv_editor_get_context()->settings.disableWatcherEventLog)
        {
            ...
            if (!(watcher->lastPathHashCode == pathHashCode &&
                  watcher->lastAccessTime.LowPart ==
                      data.lastAccessTime.LowPart &&
                  watcher->lastAccessTime.HighPart ==
                      data.lastAccessTime.HighPart))
            {
                ...
                if (FileTimeToSystemTime(&ft, &st))
                {
                    ...
                    LV_SYSTEM_LOG(debug,
                                  "Watcher Event:: Path : [%s] Action : [%s] "
                                  "LastAccessTime : [%04d-%02d-%02d "
                                  "UTC%02d:%02d:%02d.%03d]",
                                  data.path.c_str(),
                                  s_actionTypes[data.action],
                                  st.wYear,
                                  st.wMonth,
                                  st.wDay,
                                  st.wHour,
        ...
        if (currentNotiInfo->FileSize.QuadPart == 0 &&
            (currentNotiInfo->FileAttributes != FILE_ATTRIBUTE_DIRECTORY) &&
            (currentNotiInfo->Action == FILE_ACTION_MODIFIED))
        {
            continue;
        }

        if ((currentNotiInfo->FileAttributes != FILE_ATTRIBUTE_DIRECTORY) &&
            (currentNotiInfo->Action == FILE_ACTION_MODIFIED) &&
            !(currentNotiInfo->LastModificationTime.QuadPart ==
              currentNotiInfo->LastChangeTime
                  .QuadPart)) // && currentNotiInfo->LastChangeTime.QuadPart == currentNotiInfo->LastAccessTime.QuadPart))
        {
            continue;
        }
