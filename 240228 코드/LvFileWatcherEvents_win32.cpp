...
void dir_watcher_listen(LPCSTR dir,
                        LvFileWatcherEventHandler *handler,
                        LvFileWatcherEventType eventType,
                        const char *pastDir)
{
    WIN32_FIND_DATA ffd;
    ...
    do
    {
        ...
        else
        {
            ...
            {
            case LV_FILEWATCHER_EVENT_CREATE:
                handler->callback(LV_FILEWATCHER_EVENT_CREATE,
                                  path.c_str(),
                                  nullptr);
            ...
            case LV_FILEWATCHER_EVENT_MOVE:
            {
                handler->callback(LV_FILEWATCHER_EVENT_MOVE,
                                  lv_path_combine(pastDir, fileName).c_str(),
                                  path.c_str());
...
void filteredCallback(LvFileWatcherEventHandler *handler,
                      LvList<LvFileWatcherEventSet> &sets)
{
    ...
    if (!dataQueue.IsEmpty())
    {
        ...
        static LvFileWatcherEventSet
            prevDirData; //오직 잘라내기를 확인하기 위한 용도. 다른 용도로는 쓰이지 않는다.
        ...
        while (!dataQueue.IsEmpty())
        {
            ...
            if (isFile && directoriesStack.Count() > 0 &&
            ...
            {
                if (!(lv_directory_exist(data.path.c_str()) ||
                      lv_file_exist(data.path.c_str())))
                {
                    break;
                }
                ...
                if (isDirectory)
                {
                    if (prevData.action == FILE_ACTION_REMOVED &&
                        lv_path_name(prevData.path.c_str()) ==
                            lv_path_name(data.path.c_str()))
                    {
                        dir_watcher_listen(data.path.c_str(),
                                           handler,
                                           LV_FILEWATCHER_EVENT_MOVE,
                                           prevData.path.c_str());
                    }
                    else if (!directoriesSet.Contains(data.path) &&
                             (directoriesStack.Count() == 0 ||
                              data.path.IndexOf(directoriesStack.Peek().path) <
                                  0))
                    {
            ...
            case FILE_ACTION_REMOVED:
            {
                const char *nextFileName =
                    PathFindFileNameA(nextData.path.c_str());
                const char *fileName = PathFindFileNameA(data.path.c_str());

                if (strcmp(nextFileName, fileName) == 0 &&
                    nextData.action == FILE_ACTION_ADDED)
                {
                    break;
                }
                else
                {
                    //Remove
                    handler->callback(LV_FILEWATCHER_EVENT_REMOVE,
                                      data.path.c_str(),
                                      nullptr);
                }
            ...
            case FILE_ACTION_MODIFIED:
            {
                ...
                if (prevData.path == data.path &&
                    prevData.action == FILE_ACTION_ADDED)
                {
                    break;
                }
                else
                {
                    //Modify
                    handler->callback(LV_FILEWATCHER_EVENT_MODIFY,
                                      data.path.c_str(),
                                      nullptr);
                }
                break;
            ...
            case FILE_ACTION_RENAMED_NEW_NAME:
            {
                ...
                if (prevData.action == FILE_ACTION_RENAMED_OLD_NAME)
                {
                    ...
                    Engine::LvDispatchQueue::Main().Sync([&]() {
                        if (Project::LvFileManager::GetFileInfoAtAbsolutePath(
                                data.path.c_str())
                                .isValid)
                        {
                            handler->callback(LV_FILEWATCHER_EVENT_MODIFY,
        ...
        while (directoriesStack.Count() > 0)
        {
            ...
            dir_watcher_listen(set.path.c_str(),
                               handler,
                               LV_FILEWATCHER_EVENT_CREATE,
                               nullptr);
        }
...
void listen(LvFileWatcherEventHandler *handler,
...
void watch(void *t)
{
    ...
    while (false == g_shutdown)
    {
        ...
        auto eRes = WaitForMultipleObjectsEx(
            static_cast<DWORD>(set.watcher->eventList.Count()),
            set.watcher->eventList.data(),
            false,
            INFINITE,
            true);

        if (eRes >= WAIT_OBJECT_0 &&
            eRes < WAIT_OBJECT_0 + set.watcher->eventList.Count())
        {
...
LvFileWatcher *lv_file_watcher_start(LvFileWatcherCreateFlags flags,
