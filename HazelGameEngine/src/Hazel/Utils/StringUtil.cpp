...
char* StringUtil::ReplaceStr(char const* const pattern, char const* const replace, char const* const src)
{
	size_t patternCnt = 0;
	...
	for (originalPtr = src; (patternLocation = strstr(originalPtr, pattern)); originalPtr = patternLocation + patternlen)
	{
...

	{
		...
		size_t const replacedLen = originalLen + patternCnt * (replen - patternlen);
		char* const replacedChar = static_cast<char*>(malloc(sizeof(char) * (replacedLen + 1)));
...
void StringUtil::ReplaceStrToDst(char* dest, char const* const src, char const* const pattern, char const* const replace)
{
	size_t const replen = strlen(replace);
...
void StringUtil::ReplaceWString(std::wstring& str, char target, char replacement)
{
	std::replace(str.begin(), str.end(), target, replacement);
}
		...
		*/
		if (static_cast<int>(src_utf8[i]) < 0)
		{
			...
			*/
			...
			int flags = ((lower8Bit & 0x40) >> 4) + ((lower8Bit & 0x20) >> 4) + ((lower8Bit & 0x10) >> 4);
...
std::string StringUtil::pr_str_system_to_utf8(const char* cp)
{
	...
	const int bufferSize = NormalizeString(NormalizationC, inStr, static_cast<int>(cpSize), nullptr, 0);
	...
	// Normalize.
	NormalizeString(NormalizationC, inStr, static_cast<int>(cpSize), (LPWSTR)buffer.c_str(), static_cast<int>(buffer.length()));
	const int size = WideCharToMultiByte(CP_UTF8, WC_NO_BEST_FIT_CHARS, buffer.c_str(), -1, NULL, NULL, NULL, NULL);
	// LvString utf8(size);
	std::string utf8;
	utf8.resize(size);
	...
	if (size > 0)
	{
		WideCharToMultiByte(CP_UTF8, WC_NO_BEST_FIT_CHARS, buffer.c_str(), -1, (char*)utf8.c_str(), size, NULL, NULL);
		return utf8;
	}
...
std::vector<std::string> StringUtil::p_split_string(const std::string& origin, char split)
{
	char delimiter = ',';
